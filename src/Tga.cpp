...
bool    LoadUncompressedTGA(Texture *, char const *, FILE *);            // Load an Uncompressed file
bool    LoadCompressedTGA(Texture *, char const *, FILE *);              // Load a Compressed file
...
{                  // TGA Loading code nehe.gamedev.net)
    ...
    texture->height =
        tga.header[3] * 256 + tga.header[2];   // Determine The TGA Height (highbyte*256+lowbyte)
    ...
    tga.imageSize      = (tga.bytesPerPixel * tga.Width
...
bool LoadCompressedTGA(Texture * texture, char const * filename, FILE * fTGA)   // Load COMPRESSED TGAs
{
    ...
    tga.imageSize =
        (tga.bytesPerPixel * tga.Width * tga.Height);        // Compute amout of memory needed to store image
    texture->imageData = (GLubyte *)malloc(tga.imageSize);   // Allocate that much memory
    ...
    do
    {
        ...
        if(chunkheader
           < 128)   // If the ehader is < 128, it means the that is the number of RAW color packets minus 1
        {           // that follow the header
            ...
            for(short counter = 0; counter < chunkheader; counter++)   // Read RAW color values
            {
                ...
                texture->imageData[currentbyte] =
                    colorbuffer[2];   // Flip R and B vcolor values around in the process
                ...
                currentbyte +=
                    tga.bytesPerPixel;   // Increase thecurrent byte by the number of bytes per pixel
        ...
        else   // chunkheader > 128 RLE data, next color reapeated chunkheader - 127 times
        {
            ...
            {                // by the header
                texture->imageData[currentbyte] =
                    colorbuffer[2];   // switch R and B bytes areound while copying
                texture->imageData[currentbyte + 1] = colorbuffer[1];
                texture->imageData[currentbyte + 2] = colorbuffer[0];
                          ...
                          unsigned char const * data)
{
    ...
    if(depth == 3 || depth == 4)
        fwrite(TGAHeaderColor, 12, sizeof(unsigned char), tga);
    else
    {
        if(depth == 1 || depth == 2)
            fwrite(TGAHeaderBW, 12, sizeof(unsigned char), tga);
        else
        {
    ...
    write16bit(width, tga);
    write16bit(height, tga);
    ...
    switch(depth)
    {
        ...
        default:
            fprintf(stderr, "Supported component number: 1,2,3 or 4\n");
            exit(1);
    };

    if(depth == 2 || depth == 4)
        putc(0x8, tga);   // 8 bit alpha
    else
        putc(0x00, tga);

    if(depth == 1 || depth == 2)
        fwrite(data, height * width * depth, sizeof(char), tga);
    else
    {
        for(int y = 0; y < height; y++)
            for(int x = 0; x < width; x++)
            {
                fwrite(data + y * width * depth + x * depth + 2, sizeof(char), 1,
                       tga);   // Write char for BLUE
                fwrite(data + y * width * depth + x * depth + 1, sizeof(char), 1,
                       tga);   // Write char for GREEN
                fwrite(data + y * width * depth + x * depth + 0, sizeof(char), 1,
                       tga);   // Write char for RED
                if(depth == 4)
                    fwrite(data + y * width * depth + x * depth + 3, sizeof(char), 1,
                           tga);   // Write char for ALPHA
            }
    }
