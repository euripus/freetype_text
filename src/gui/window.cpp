...
void UIWindow::loadWindowFromDesc(std::string const & file_name)
{
    ...
    if(auto const & win_obj = jv.get_object(); !win_obj.empty())
    {
        ...
        {
            if(kvp.key() == UIWindow::sid_window_size)
            {
                std::vector<int32_t> vec;
                vec = boost::json::value_to<std::vector<int32_t>>(kvp.value());

                m_rect.m_size.x = static_cast<float>(vec[0]);
                m_rect.m_size.y = static_cast<float>(vec[1]);
            }
            else if(kvp.key() == UIWindow::sid_window_caption)
            {
            ...
            else if(kvp.key() == UIWindow::sid_widgets)
            {
                ...
                if(!arr.empty())
                {
                    ...
                    m_root                  = Widget::GetWidgetFromJson(root_entry.as_object(), *this);

                    auto const & background_entry = arr[1];
                    m_background = Widget::GetWidgetFromJson(background_entry.as_object(), *this);
                }
            }
        }
    }
