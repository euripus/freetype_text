...
enum class FontLocation
    ...
    float GlyphGetKerning(Glyph const & glyph, const std::uint32_t charcode) const;   // charcode  codepoint of the peceding glyph
protected:
private:
    bool InitFont();
    ...
    float _height; // This field is simply used to compute a default line spacing (i.e., the baseline-to-baseline distance) when writing text with this font.
    float _linegap; //This field is the distance that must be placed between two lines of text. The baseline-to-baseline distance should be computed as: ascender - descender + linegap
    float _ascender; // The ascender is the vertical distance from the horizontal baseline to the highest 'character' coordinate in a font face.
    float _descender; // The descender is the vertical distance from the horizontal baseline to the lowest 'character' coordinate in a font face.
