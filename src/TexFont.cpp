...
std::int32_t TexFont::textureFontLoadGlyph(std::uint32_t ucodepoint)
{
    ...
    {
        Glyph      glyph;
        size_t     size   = m_atlas.getSize();
        glm::ivec4 region = m_atlas.getRegion(5, 5);
        ...
        m_atlas.setRegion(glm::ivec4(region.x, region.y, 4, 4), data, 0);
    ...
    x = region.x;
    y = region.y;
    m_atlas.setRegion(glm::ivec4(x, y, w, h), ft_bitmap.buffer, ft_bitmap.pitch);
